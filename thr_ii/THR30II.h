...
// Structure to describe a numeric key and its friendly name
struct key_name
{
	uint16_t key;
	String name;
...
// Structure to describe a parameter setting
struct unit_info
{
 uint16_t uk; // unit key
 uint16_t sk; // subunit key
 String na;   // parameter name
...
class THR30II_Settings
{
  ...
  // Using a template function to be able to use it for different types
	...
	std::array<double, CTRL_TREBLE - CTRL_GAIN + 1> control       {{50, 50, 50, 50, 50}}; // Actual state of main control knobs (initialized with default values)
...
//    SysExMessage(SysExMessage &&other ) noexcept ; // Move Constructor
    SysExMessage(SysExMessage &&other ) ; // Move Constructor
