...
String THR30II_Settings::ParseSysEx(const byte cur[], int cur_len)  
{
    ...
    if (cur[2] == 0x01 && cur[3] == 0x0c && cur[4] == 0x24)   //Valid SysEx start : THR30II (LINE6 MIDI-ID )
    {
        ...
        // Should familyID be 0x014d for THR10II_W?
        ...
        //{
        //  res += " ";
        //  res += String(cur[ii], HEX);
        //}
        //Serial.println(res);

        if (familyID == 0x014d)  // Valid Message for THR10II_W received?
//        if (familyID == 0x024d)  // Valid Message for THRII received
        ...
        {        //cur[5], cur[6]
            if (familyID == 0x017E)  //01 7e  +(7f 06 02)  (not a family ID but the 2nd answer frame to identity request)
//            if (familyID == 0x027E)  //02 7e  +(7f 06 02)  (not a family ID but the 2nd answer frame to identity request)
        ...
        // TODO: Hardcoded famID (24) and modNr (02)
        // THR10II_W modNr=01. Assume that this has to be used in communication, not the hard-coded 02
                        ...
                        {   
                            ...
                            if (outqueue.itemCount() > 0)
                            {
                                ...
                                if (id == 7)  //only react, if it was the question #7 from the boot-up dialog ("have user settings changed?")
                                {
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage( (const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x01, 0x02, 0x00, 0x00, 0x0b, 0x00, 0x0c, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x3c, 0x00, 0x7f, 0x7f, 0x7f, 0x7f, 0x00, 0x00, 0xf7 } ,29), 8, false, true));
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x04, 0x00, 0x00, 0x07, 0x00, 0x0d, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 9, false, false));
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage( (const byte[21]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x05, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },21), 10, false, true));
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x01, 0x03, 0x00, 0x00, 0x0b, 0x00, 0x06, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 11, false, true));
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x01, 0x04, 0x00, 0x00, 0x0b, 0x00, 0x06, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 12, false, true));
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[29]){ 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x01, 0x05, 0x00, 0x00, 0x0b, 0x00, 0x06, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 13, false, true));
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x01, 0x06, 0x00, 0x00, 0x0b, 0x00, 0x06, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29),14,false,true));
                                    ...
                                    //answer will be the name of user-setting 4
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x01, 0x07, 0x00, 0x00, 0x0b, 0x00, 0x06, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 15, false, true));
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x08, 0x00, 0x00, 0x07, 0x00, 0x0d, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 16, false, false)); //Header!
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[21]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x09, 0x00, 0x00, 0x03, 0x00, 0x0b, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },21),17, false, true)); //answer will be 01, 0c, 00, 02, 00=extracted / 02=plugged in
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x2a, 0x00, 0x00, 0x07, 0x00, 0x0d, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 18, false, false)); //Header!;
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x2b, 0x00, 0x00, 0x03, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 19, false, true)); 
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x06, 0x00, 0x00, 0x07, 0x00, 0x09, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 20, false, false)); //Header!
                                    ...
                                    char tmp [29]= { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x07, 0x00, 0x00, 0x07, 0x78, 0x7f, 0x7f, 0x7f, 0x7f, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 };
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x2c, 0x00, 0x00, 0x07, 0x00, 0x0d, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 22, false, false)); //Header!
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[21]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x2d, 0x00, 0x00, 0x03, 0x00, 0x0e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },21), 23, false, true)); //Answer will be 01, 0c, 00, 02, 00=open/01=focus;
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x06, 0x00, 0x00, 0x07, 0x00, 0x09, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 24, false, false)); //Header!;
                                    ...
                                    byte tmp1[29]= { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x07, 0x00, 0x00, 0x07, 0x78, 0x7f, 0x7f, 0x7f, 0x7f, 0x55, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 };
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage((const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x06, 0x00, 0x00, 0x07, 0x00, 0x09, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 26, false, false)); //Header!
                                    ...
                                    char tmp2[29]= { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x07, 0x00, 0x00, 0x07, 0x78, 0x7f, 0x7f, 0x7f, 0x7f, 0x4b,0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 };
                    ...
                    switch (msgVals[2]) // Block-Key / TargetTypeKey
                    {
                        ...
                        case 0x01440061ul:   //latest Firmware 1.44.0a
                            ...
                            {   //use the 2nd "Req. Firmware"
                                ...
                                outqueue.enqueue(Outmessage(SysExMessage((const byte[29]){ 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x03, 0x00, 0x00, 0x07, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7},29), 777, false, true));
                            ...
                            else if (writeOrrequest == 0) //followed a "Req. Firmware" with the "00"-Marker
                            {
                                ...
                                if (Firmware == 0x01300063)  //old firmware 1.30.0c
                                {
                                    ...
                                    outqueue.enqueue(Outmessage(SysExMessage( (const byte[21]){ 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x02, 0x00, 0x00, 0x03, 0x60, 0x6b, 0x3e, 0x6f, 0x68, 0x00, 0x00, 0x00, 0xf7 },21), 4, true, false));
                                ...
                                else if (Firmware == 0x0131006B) //new firmware 1.31.1k
                                {
                                    ...
                                    //send Midi activation and await ack
                                    outqueue.enqueue(Outmessage(SysExMessage( (const byte[21]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x02, 0x00, 0x00, 0x03, 0x28, 0x24, 0x6b, 0x09, 0x18, 0x00, 0x00, 0x00, 0xf7 },21), 4, true, false));
                                ...
                                outqueue.enqueue(Outmessage(SysExMessage((const byte[29]){ 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x01, 0x00, 0x00, 0x00, 0x07, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 5, false, true));
                ...
                if (cur_len >= 0x1a)  // 00 01 07 - messages have to be longer than 0x2c bytes (failure otherwise)
                {
                    ...
                    if (msgVals[0] == 2 && msgVals[1] == 0x10)  //User-Setting change report
                    {
                            if(msgVals[3]<5)   //a user setting was changed on THR (pressed one of knobs "1"..."5")
                            {
                                ...
                                userSettingsHaveChanged = false;
                                thrSettings = false; // Important only during the first user setting activation (by pressing one of the iser memory buttond on the amp)
...
//                                outqueue.enqueue(Outmessage(SysExMessage((const byte[29]){ 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x01, 0x02, 0x00, 0x00, 0x0b, 0x00, 0x0c, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 88, false, true)); //answer will be the settings dump for actual settings
                    ...
                    {
                        TRACE_THR30IIPEDAL(result += String("\n\rParameter change report: ");)
                ...
                else //Not the identification string
                {
                    if (payloadSize <= 0x100)   //framelen other than 9, 12,16,20, 24 Bytes but shorter than 0x100Bytes (will be a dump)  
                    {
                        ...
                        else if (dumpInProgress && (sameFrameCounter == dumpFrameNumber + 1))  //found next frame of a dump
                        {
                            ...
                            TRACE_THR30IIPEDAL(result += " Chunk " +S tring(dumpFrameNumber + 1) + " of a dump: Size of this chunk's payload= " + String(payloadSize - 1, HEX);)
                        }
                        else
                        {
                        ...
                        if (dumpInProgress && (dumpByteCount >= dump_len)) //fetched enough bytes for complete dump
                        {
                            ...
                            else if (symboldump)
                            {
                                ...
                                outqueue.enqueue(Outmessage(SysExMessage((const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x01, 0x01, 0x00, 0x00, 0x07, 0x00, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 6, false, true));
                                ...
                                outqueue.enqueue(Outmessage(SysExMessage( (const byte[29]) { 0xf0, 0x00, 0x01, 0x0c, 0x24, 0x01, 0x4d, 0x00, 0x03, 0x00, 0x00, 0x07, 0x00, 0x0f, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf7 },29), 7, false, true));
    ...
    else
    {
        ...
        {
            // TODO: Somehow use the extracted famID and modNr in the rest of the communication?
            // Currently it is hardcoded as 24 and 02 (THR30II)
            uint16_t famID = 0;
